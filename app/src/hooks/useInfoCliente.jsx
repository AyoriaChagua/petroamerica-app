...
export const useInfoCliente = () => {
    ...
    const mostrarVentanaEmergente = async (tipoDoc, titulo) => {
        ...
        let documentos;
        if (tipoDoc === "xdeudastotal") documentos = await HttpInfoCliente.docDeudasTotal(token, cliente.id_cliente);
        else documentos = await HttpInfoCliente.docConSaldo(token, cliente.id_cliente, tipoDoc);
        if (documentos.error) {
    ...
    const agregarPlantaAlFiltro = (planta) => {
        ...
        let sumatoria = 0;
        if (planta === "") {
            sumatoria = docConSaldo.reduce((total, item) => {
                if (isChecked && parseFloat(item['cantidad']) >= 1.00) {
            ...
            const nuevoDocumento = docConSaldo.filter(value => value.planta === planta);
            sumatoria = nuevoDocumento.reduce((total, item) => {
                if (isChecked && parseFloat(item['cantidad']) >= 1.00) {
                    return total + parseFloat(item['cantidad']);
                } else if (!isChecked) {
                    return total + parseFloat(item['cantidad']);
        ...
        const sumatoriaFormateada = curencyformatter.format(sumatoria, { code: '', precision: 2 });
        setSaldoTotalDoc(sumatoriaFormateada);
    
        // Detener la carga de información después de un tiempo
        setTimeout(() => {
            setCargaInformacion(false);
        }, 500);
    };
    ...
    const handleCheckboxChange = (value) => {
        ...
        let sumatoria = docConSaldo.reduce((total, item) => {
            if (value && parseFloat(item['cantidad']) >= 1.00) {
                return total + parseFloat(item['cantidad']);
            } else if (!value) {
                return total + parseFloat(item['cantidad']);
            }
    ...
    const cerrarModalDocMes = () => {
        setVerModalDocMes(false);
        setIsChecked(false);
    }
